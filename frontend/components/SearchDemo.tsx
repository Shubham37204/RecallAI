...
export default function SearchDemo() {
  ...
  return (
    <section id="search-demo" className="w-full bg-zinc-950 py-20 border-t border-violet-950/60">
          ...
          <Tabs defaultValue="search" className="w-full">
            ...
            <div className="border-b border-zinc-900 px-4 py-2 flex items-center justify-between">
              <TabsList className="bg-zinc-900 border-none p-0.5 rounded-md h-8">
                <TabsTrigger value="search" className="text-[11px] h-7 px-3 rounded text-zinc-400 data-[state=active]:bg-violet-650 data-[state=active]:text-white hover:text-zinc-200 transition-colors">
                  <Search className="h-3 w-3 mr-1" />
                  Search
                </TabsTrigger>
                <TabsTrigger value="summaries" className="text-[11px] h-7 px-3 rounded text-zinc-400 data-[state=active]:bg-violet-650 data-[state=active]:text-white hover:text-zinc-200 transition-colors">
                  <Sparkles className="h-3 w-3 mr-1" />
                  Summaries
                </TabsTrigger>
                <TabsTrigger value="tags" className="text-[11px] h-7 px-3 rounded text-zinc-400 data-[state=active]:bg-violet-650 data-[state=active]:text-white hover:text-zinc-200 transition-colors">
                  <Tag className="h-3 w-3 mr-1" />
                  Tags
                </TabsTrigger>
                <TabsTrigger value="collections" className="text-[11px] h-7 px-3 rounded text-zinc-400 data-[state=active]:bg-violet-650 data-[state=active]:text-white hover:text-zinc-200 transition-colors">
            ...
            <TabsContent value="search" className="p-6">
              <div className="max-w-2xl mx-auto flex flex-col gap-4">
                ...
                <span className="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider"> Results for &ldquo;{searchQuery}&rdquo;</span>
                  ...
                  {[
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-violet-400 font-medium">{res.url}</span>
                        <span className="text-[10px] text-emerald-400 bg-emerald-950/40 border border-emerald-900/80 px-1.5 py-0.5 rounded font-mono">{res.match}</span>
                      </div>
                ...
                <div className="mt-3 space-y-3">
                  ...
                  <div>
                    ...
                    <ul className="list-inside text-xs text-zinc-400 mt-1 space-y-1">
                      ...
                      <li className="flex items-start gap-1.5">
                        <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-violet-500" />
                       <span>Doesn&apos;t trigger component refresh on non-selected states</span>
                      </li>
                      <li className="flex items-start gap-1.5">
                        <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-violet-500" />
                        <span>Transient updates support (state changes without render cycles)</span>
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <TabsContent value="tags" className="p-6">
              <div className="max-w-md mx-auto grid grid-cols-2 gap-3">
                {[
                  ...
                  if (item.tag === "React 19") style = "border-blue-900/50 bg-blue-950/20 text-blue-300";
                  if (item.tag === "State Management") style = "border-purple-900/50 bg-purple-950/20 text-purple-300";
                  if (item.tag === "CSS Grid") style = "border-pink-900/50 bg-pink-950/20 text-pink-300";
                  if (item.tag === "Database Indexing") style = "border-amber-900/50 bg-amber-950/20 text-amber-300";
                  return (
                    <div key={idx} className="border border-zinc-900 rounded-lg p-3 bg-zinc-900/20 text-left hover:border-violet-900/40 transition-colors">
                      <div className="flex items-center justify-between">
                        <Badge variant="outline" className={`text-[10px] font-mono border-none ${style}`}>
                          #{item.tag}
                        </Badge>
